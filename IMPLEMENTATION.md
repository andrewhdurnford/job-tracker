...
- If drift becomes intolerable, the escape hatch is a Fly.io machine or a local `launchd` timer running the same `poll.js` against the same files — the script has no Actions-specific code.
